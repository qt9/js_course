...
Battleground.prototype.areaDamage = function (activeRobot) {
    for (var A = 0; A < this.robots.length; A++) {
        if (activeRobot != this.robots[A] && this.robots[A].alive) {
            activeRobot.attack(this.robots[A]);
            this.updateAliveStatus(this.robots[A]);
        }
    }
};
